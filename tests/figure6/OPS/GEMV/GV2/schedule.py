...
BPad_shared = s.cache_read(BPad, shared, dense.wmma.accumulator)
A_shared_wmma_matrix_a = s.cache_read(A_shared, wmma.matrix_a, dense.wmma.accumulator)
BPad_shared_wmma_matrix_b = s.cache_read(BPad_shared, wmma.matrix_b, dense.wmma.accumulator)
...
#==--------- Start schedule STAGE BPad.shared.wmma.matrix_b ----------==#
s[BPad_shared_wmma_matrix_b].compute_at(s[dense_wmma_accumulator], k_o)

# Var ax0
# Var ax1
## Tensor core loadB
ax0_o, ax0_i = s[BPad_shared_wmma_matrix_b].split(ax0, factor = 16)
ax1_o, ax1_i = s[BPad_shared_wmma_matrix_b].split(ax1, factor = 16)
s[BPad_shared_wmma_matrix_b].reorder(ax0_o, ax1_o, ax0_i, ax1_i, )
s[BPad_shared_wmma_matrix_b].tensorize(ax0_i, intrin_wmma_load_matrix_W(
...
# Var ax0_i length 1
# Var ax1_i length 1
#==--------- Start schedule STAGE BPad.shared ----------==#
s[BPad_shared].compute_at(s[dense_wmma_accumulator], k_o)

# Var ax0 length 1
# Var ax1 length 1
## Storage align 
s[BPad_shared].storage_align(ax0, 0.000000, 1.000000)
ax0_ax1_f = s[BPad_shared].fuse(ax0, ax1, )
ax0_ax1_f_o, ax0_ax1_f_i = s[BPad_shared].split(ax0_ax1_f, factor = 1)
s[BPad_shared].vectorize(ax0_ax1_f_i)
ax0_ax1_f_o_o, ax0_ax1_f_o_i = s[BPad_shared].split(ax0_ax1_f_o, factor = 1)
s[BPad_shared].bind(ax0_ax1_f_o_i, te.thread_axis("threadIdx.x"))
ax0_ax1_f_o_o_o, ax0_ax1_f_o_o_i = s[BPad_shared].split(ax0_ax1_f_o_o, factor = 1)
s[BPad_shared].bind(ax0_ax1_f_o_o_i, te.thread_axis("threadIdx.y"))
